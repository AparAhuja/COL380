...
if __name__ == "__main__":
    ...
    for block1 in mat1:
        ...
        for block2 in mat2:
            ...
            if y1 != x2:
                continue
            # multiply the blocks and add to block3.
            product = block1[2] @ block2[2]
            # if the output matrix is all zeros.
            if not np.any(product):
                continue
            # add the block to mat3 with coordinates as (x1, y2). Clip the values.
            if (x1, y2) not in mat3.keys():
                # first block for row of mat1 and col of mat2.
                mat3[(x1, y2)] = np.clip(product, a_min=0, a_max=MAX).astype(np.uint32)
                k3 += 1
            else:
                # another block found for row of mat1 and col of mat2.
                mat3[(x1, y2)] = np.clip(mat3[(x1, y2)] + product, a_min=0, a_max=MAX).astype(np.uint32)
